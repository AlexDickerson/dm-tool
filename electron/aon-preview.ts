 ...
 */
export async function fetchAonPreview(
  urlPath: string,
): Promise<AonPreviewData | null> {
  try {
    const res = await fetch(AON_URL, {
      ...
      body: JSON.stringify({
        ...
        size: 1,
        _source: [
          "name", "category", "text", "url", "summary",
          // Creature fields
          "level", "hp", "hp_raw", "ac", "fortitude_save", "reflex_save",
          "will_save", "perception", "speed_raw", "size", "trait_raw",
          "creature_ability", "creature_family", "immunity", "weakness_raw",
          "resistance", "rarity",
    ...
    if (hit.category === "creature") {
      ...
      const statBlock = sections.length > 1
